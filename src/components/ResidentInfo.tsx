import { ResidentProps } from "../interface/interfaces";

const ResidentInfo = ({ resident }: ResidentProps) => {
  return (
    <div
    ...
    >
      ...
      <section className="flex flex-col text-start">
        <h2 className="font-bold w-full border-b-2 border-black">{resident.name}</h2>
        <span><b>Species: </b> {resident.species}</span>
        <span><b>Type: </b> {resident.type === "" ? "unknown" : resident.type}</span>
        <span><b>Gender: </b> {resident.gender}</span>
        <span><b>Episodes:</b> {resident.episode.length}</span>
      </section>
    </div>
  );
};
